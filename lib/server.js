...
class Server {
  ...
  listen() {
    const { routesToController, config } = this;
    const { port, eagerVerifyDb } = config;
    this.app.listen(port, () => {
      this.log(`👂 listening on port ${chalk.cyanBright(port)}`, true);
      if (eagerVerifyDb) {
        const verifiers = [...routesToController.values()].map((controller) => {
          return controller
            .verifyDb()
            .then((count) =>
              this.log(`✔️  found ${count} ${controller.tableName}`)
            );
        });
        Promise.all(verifiers).then(() => {
          this.log(chalk.greenBright('verified all resources'));
        });
      }
    });
  }
