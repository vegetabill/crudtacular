...
const { keyBy } = require('../util');
const { createColumn } = require('./column-factory');
...
class Schema {
  ...
  wrapForeignKeyColumns(tableMap, foreignKeys) {
    foreignKeys.forEach((fk) => {
      let table = tableMap.get(fk.origin.table);
      let column = table.columns.get(fk.origin.column);

      let foreignTable = tableMap.get(fk.destination.table);
      let foreignColumn = foreignTable.columns.get(fk.destination.column);

      table.columns.set(
        fk.origin.column,
        new ForeignKeyColumn(column, foreignColumn)
      );
    });
  ...
  getColumnMap(tableName) {
    return bluebird.join(
      ...
      (columns, primaryKey) => {
        ...
        const wrappedPk = new PrimaryKeyColumn(all.get(primaryKey));
        all.set(primaryKey, wrappedPk);
        return all;
      }
    );
  }
  ...
  getColumns(tableName) {
    return bluebird.map(
      ...
      ({ column_name, data_type, is_nullable }) =>
        createColumn(tableName, column_name, data_type, is_nullable === 'YES')
    );
  }
